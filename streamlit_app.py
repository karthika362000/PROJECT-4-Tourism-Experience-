...
def load_data():
    ...
    ContinentId, RegionId, CountryId, CityId, Country,Region, Continent, AttractionTypeId, Attraction,
...
def main():
    ...
    if app_mode == "Trend Analysis":
        ...
        with col2:
            ...
            sns.barplot(x=top_attractions.values, y=top_attractions.index, ax=ax, hue=top_attractions.index, palette='viridis', legend=False)
        ...
        def generate_synthetic_data():
            ...
            attractions = {
                'India': ['Taj Mahal', 'Jaipur Palace', 'Goa Beaches'],
                'Indonesia': ['Bali Temples', 'Komodo Island'],
                'Thailand': ['Phuket', 'Chiang Mai Temples'],
                'Japan': ['Mount Fuji', 'Tokyo Skytree'],
                'France': ['Eiffel Tower', 'Louvre Museum'],
                'Italy': ['Colosseum', 'Venice Canals'],
                'Spain': ['Sagrada Familia', 'Madrid Royal Palace'],
                'USA': ['Statue of Liberty', 'Grand Canyon']
            }
            return countries, attractions

        countries, attractions = generate_synthetic_data()

        # Allow users to select country and get recommendations
        country = st.selectbox("Select Country", countries)
        recommendations = get_recommendations(df, country=country)
